...
{\tt nclust           }& number of QM electrons - if set to 0 or a negative value (charge)  this will be automatically calculated\\
...
{\tt dx,dy,dz,        }& grid spacing (in  Bohr) for the 3D numerical grid - if negative this will be set to an optimal value and a value will
be suggested for KXBOX in file NX - the code stops and has to be restarted\\
...
{\tt ipsptype         }& type of pseudopotentials: 0 = soft local (errf);\\
                       ...
                       & 3 = read from file goed.asci (no need to specify)  ; 4 = semicore read from file goed.asci\\
