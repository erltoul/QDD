...
COMPLEX(DP),DIMENSION(KDFULL2) :: Q1
COMPLEX(DP) :: cdtact
COMPLEX(DP),DIMENSION(KDFULL2,KSTATE) :: X,Y,ERRCN
...
DO nb=1,nstate
   CALL exp_evol(q0(1:kdfull2,nb),aloc,nb,1,cdtact,q1)
END DO
...
!!$DO nb=1,nstate
!!$  !  qwork(:,nb) = q0(:,nb)
!!$  CALL exp_evol(q0(:,nb),aloc,nb,1,cdtact,q1)
!!$END DO
...
!!$if(level.ne.0) then
!!$  do nb=1,nstate
!!$    call mpi_recv(q0(:,nb),kdfull2,mpi_double_complex,level-1,1,mpi_comm_World,is,icode)
!!$  enddo
...
!!$do nb=1,nstate
!!$
!!$call from1Dto3Dc(q0(:,nb),qarrayfine,nx2,ny2,nz2)
...
!!$if(level.ne.nprocs-1) then
!!$do nb=1,nstate
!!$call mpi_recv(q0(:,nb),kdfull2,mpi_double_complex,level+1,2,mpi_comm_World,is,icode)
!!$enddo
!!$endif
!!$
!!$
...
!!$if(level.ne.0) then 
!!$do nb=1,nstate
!!$call from1Dto3Dc(q0(:,nb),qarray,nx2,ny2,nz2)    !from coarse vector to coarse array
